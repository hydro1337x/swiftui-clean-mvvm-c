...
class FakeRepository: FetchStoriesRepository, FetchPostsRepository {
  ...
  func fetch(_ request: Domain.FetchStoriesInput) async -> Result<[Domain.Story], Error> {
    if request.isInitial {
      storiesPaginator.reset()
    }
    
    let page = storiesPaginator.paginate(makeStoriesPage())
  ...
  func fetch(_ request: Domain.FetchPostsRequest) async -> Result<[Domain.Post], Error> {
    ...
    let page = postsPaginator.paginate(makePostsPage())
    
    return .success(page)
  }
  
  func makeStoriesPage() -> PaginatedResponse<Story> {
    let rawPage = Array(repeating: 1, count: 8)
      .enumerated()
      .map { index, _ in
        Story(
          id: (storiesPaginator.cursor + index).description,
          location: .init(
            id: (storiesPaginator.cursor + index).description,
            name: "Venue Name \(storiesPaginator.cursor + index)",
            address: "Address \(storiesPaginator.cursor + index)",
    ...
    return PaginatedResponse(page: rawPage, pagination: .init(cursor: storiesPaginator.cursor, hasNext: true))
  }
  
  func makePostsPage() -> PaginatedResponse<Post> {
    let rawPage = Array(repeating: 1, count: 8)
      .enumerated()
      .map { index, _ in
        Post(
          id: (postsPaginator.cursor + index).description,
          name: "Post \(postsPaginator.cursor + index)",
          description: "Description \(postsPaginator.cursor + index)",
          medias: [.init(url: URL(string: postImageURLs[index])!, isFavorite: true)],
          location: .init(
            id: (postsPaginator.cursor + index).description,
            name: "Venue Name \(postsPaginator.cursor + index)",
            address: "Address \(postsPaginator.cursor + index)",
            medias: [.init(url: URL(string: avatarImageURLs[index])!, isFavorite: true)]),
